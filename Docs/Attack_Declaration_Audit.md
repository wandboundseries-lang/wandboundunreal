...
- Friendly targets are normally illegal in Godot unless the friendly target is frozen. Frozen breaking is damage-resolution behavior, so this Unreal pass treats friendly targets as illegal.
...
- Legal attacks require active normal-turn priority, attacker ownership, enemy or neutral target, `AttacksLeft > 0`, attack-capable status, orthogonal alignment, AR range, and unblocked LOS.
...
- No damage, frozen breaking, pending attack, response window, counters, passives, cards, terrain modifiers, wands, UI, VFX, or 3D runtime are implemented in this pass.
