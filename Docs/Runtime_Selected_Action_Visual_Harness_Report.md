...
`WBRuntimeActionSelectionBridge` now sits upstream of the facade/harness path. It resolves selected action IDs from externally supplied legal action lists, then delegates the resolved `FWBAction` to the facade; the harness still only executes an already selected action.
