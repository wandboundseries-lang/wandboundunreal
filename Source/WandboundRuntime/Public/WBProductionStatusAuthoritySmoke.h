...
struct WANDBOUNDRUNTIME_API FWBProductionStatusAuthoritySmokeResult
{
	bool bOk = false;
	FString Reason;
...
class WANDBOUNDRUNTIME_API WBProductionStatusAuthoritySmoke
{
public:
	static bool IsRequested(const TCHAR* CommandLine = nullptr);
	static FString GetReceiptPath();
	static FWBProductionStatusAuthoritySmokeResult Run(
