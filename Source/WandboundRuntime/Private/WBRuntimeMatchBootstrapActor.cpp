	...
	AWBRuntimePlayerController* LocalController)
{
	...
	if (bProductionDataMode)
	{
		...
		else if (WBProductionCSNCrashInSmoke::IsPatchRequested())
		{
			...
			if (!PatchSmoke.bOk)
			{
				UE_LOG(LogWBRuntimeLocalPlay, Error,
					TEXT("Wandbound CSN Patch smoke failed: %s"), *PatchSmoke.Reason);
			}
			FPlatformMisc::RequestExitWithStatus(
				false, PatchSmoke.bOk ? 0 : 37, TEXT("WandboundProductionCSNPatchSmoke"));
		}
		else if (WBProductionCSNCrashInSmoke::IsVexRequested())
		{
		...
		else if (WBProductionNPCReactionCombatSmoke::IsRequested())
		{
			...
			FPlatformMisc::RequestExitWithStatus(
				false,
				NPCCombatSmoke.bOk ? 0 : 28,
				TEXT("WandboundProductionNPCReactionCombatSmoke"));
		}
		else if (WBProductionSuspendedAttackSmoke::IsRequested())
		{
			const FWBProductionSuspendedAttackSmokeResult SuspendedAttackSmoke =
				WBProductionSuspendedAttackSmoke::Run(PendingBootstrapRequest);
			FPlatformMisc::RequestExitWithStatus(
				false,
		...
		else if (WBProductionCardZoneTransitionSmoke::IsRequested())
		{
			const FWBProductionCardZoneTransitionSmokeResult TransitionSmoke =
				WBProductionCardZoneTransitionSmoke::Run(
					PendingBootstrapRequest);
			if (!TransitionSmoke.bOk)
			{
				UE_LOG(
					LogWBRuntimeLocalPlay,
					Error,
					TEXT("Wandbound card zone transition smoke failed: %s"),
					*TransitionSmoke.Reason);
			}
			FPlatformMisc::RequestExitWithStatus(
				false,
		...
		else if (WBProductionReactionWindowSmoke::IsRequested())
		{
			...
			FPlatformMisc::RequestExitWithStatus(
				false,
				ReactionSmoke.bOk ? 0 : 25,
				TEXT("WandboundProductionReactionWindowSmoke"));
		}
		else if (WBProductionHybridNonHeroSmoke::IsRequested())
		{
			const FWBProductionHybridNonHeroSmokeResult HybridSmoke =
				WBProductionHybridNonHeroSmoke::Run(PendingBootstrapRequest);
			FPlatformMisc::RequestExitWithStatus(
				false,
				HybridSmoke.bOk ? 0 : 24,
				TEXT("WandboundProductionHybridNonHeroSmoke"));
		}
		else if (WBProductionHybridReplacementSmoke::IsRequested())
		{
